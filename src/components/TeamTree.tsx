...
function buildTree(members: TeamMember[]): TreeNode[] {
  ...
  // If there are multiple root nodes, wrap them under a virtual root
...
function OrgNode({
  ...
  // Special case: Virtual root with Directors who have grandchildren

  // Render Directors without connectors, then all grandchildren with single horizontal line

  if (isVirtual && children.length > 1) {

    const allGrandchildren = children.flatMap(child => child.children);

    const hasGrandchildren = allGrandchildren.length > 0;
    ...
    return (
      ...
      <div className="flex flex-col items-center">
        ...
        {/* Render Directors side by side with individual vertical connectors */}

        <div className="flex flex-wrap items-start justify-center gap-12 sm:gap-16 md:gap-20 lg:gap-24">
          ...
          {children.map((child) => (
            ...
            <div key={child._id} className="flex flex-col items-center">
              ...
              <ConnectorVertical className="h-6" />
        ...
        {/* If Directors have grandchildren, render them with single horizontal connector */}

        {hasGrandchildren && (

          <>

            {/* Horizontal line connecting Directors' vertical lines */}

            <ConnectorHorizontalResponsive width={Math.min(children.length * 180, 400)} />

            {/* Vertical line from center to second level */}

            <ConnectorVertical className="h-8" />

            {/* Horizontal line for all grandchildren - will span full width of children */}

            {allGrandchildren.length > 1 && (
            ...
            <div className="flex flex-nowrap items-start justify-center gap-3 sm:gap-4 md:gap-5 lg:gap-6 w-full">

              {children.map((director) =>

                director.children.map((grandchild) => (

                  <div key={grandchild._id} className="flex flex-col items-center">

                    <ConnectorVertical className="h-5" />
  ...
  return (
    ...
    <div className="flex flex-col items-center px-2">
      ...
      {!isVirtual && <MemberCard member={node} size={size} onSelect={onSelect} />}





      {children.length > 0 && !skipChildren && (
        ...
        <>
          ...
          {!isVirtual && <ConnectorVertical className="h-10" />}
          ...
          <div className="flex flex-nowrap items-start justify-center gap-4 sm:gap-6 md:gap-8 lg:gap-10 w-full">
