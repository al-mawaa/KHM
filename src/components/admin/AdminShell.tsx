...
const menu = [
  ...
  // Team Members page temporarily disabled
...
export function AdminShell({ title, children }: { title: string; children: ReactNode }) {
  ...
  if (!ready) {
    return (
      <div className="min-h-screen grid place-items-center bg-slate-950 text-white">Loading…</div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-72 bg-slate-950 text-slate-100 transform transition-transform duration-300 lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}
      >
        ...
        <button
          ...
          className="absolute bottom-0 inset-x-0 flex items-center gap-3 px-5 h-14 border-t border-white/10 text-sm font-medium text-red-300 hover:bg-red-500/10"
        >
        ...
        <header className="sticky top-0 z-20 bg-white/80 backdrop-blur border-b border-slate-200">
          <div className="flex items-center justify-between px-4 sm:px-8 h-16">
            ...
            <div className="flex items-center gap-3">
              <Link
                to="/"
                className="text-xs font-medium text-slate-600 hover:text-aqua-foreground"
              >
                ← View Site
              </Link>
              <div className="grid h-9 w-9 place-items-center rounded-full bg-gradient-aqua text-primary-foreground text-xs font-bold">
                AD
              </div>
            </div>
          </div>
