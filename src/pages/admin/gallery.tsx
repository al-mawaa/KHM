  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    const toAdd = await Promise.all(
      ...
      Array.from(files).slice(0, 12).map(
        ...
        (f) => new Promise<{ title: string; category: string; image: string; albumName: string; description: string }>((res) => {
  ...
  return (
    ...
    <AdminShell title="Manage Gallery">
      ...
      {loading ? (
        ...
        <p className="text-center text-slate-500 py-12">No images yet — upload to get started.</p>
