...
import { GALLERY_CTA, GALLERY_HERO, GALLERY_ITEMS } from "@/lib/gallery-content";
...
export default function GalleryPage() {
  ...
  const items = GALLERY_ITEMS[activeSection];
      ...
      <section className="py-12 lg:py-16">
        <div className="mx-auto max-w-[1400px] px-4 lg:px-6">
          ...
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((item) => (
              <article
                key={`${activeSection}-${item.caption}`}
              ...
              >
                ...
                <p className="py-3 text-center text-sm font-semibold text-[#1a5276]">{item.caption}</p>
              </article>
            ))}
          </div>
        </div>
