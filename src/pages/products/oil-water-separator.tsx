...
import { OIL_WATER_SEPARATOR } from "@/lib/products/oil-water-separator";

export default function OilWaterSeparatorPage() {
  const { valueImages, detailBlocks, faqs } = OIL_WATER_SEPARATOR;
    ...
    <main className="bg-white text-gray-800">
      ...
      <div className="border-b border-gray-200 bg-[#f4f6f8]">
        <div className="mx-auto flex max-w-[1400px] items-center gap-2 px-4 py-3 text-xs font-medium uppercase tracking-wide text-gray-600 lg:px-6">
          ...
          <ChevronRight className="h-3.5 w-3.5 opacity-50" aria-hidden />
          <span className="text-[#1a5276]">Oil Water Separator</span>
        </div>
      </div>
      ...
      <section className="py-12 lg:py-16">
        <div className="mx-auto max-w-[1400px] px-4 lg:px-6">
          ...
          >
            <img
              src={OIL_WATER_SEPARATOR.heroImage}
              alt="Oil water separator — wastewater treatment facility"
              className="product-img-banner w-full"
              loading="eager"
            />
          </motion.div>
          ...
          >
            <h1 className="font-display text-2xl font-bold uppercase tracking-wide text-[#1a5276] sm:text-3xl lg:text-4xl">
              {OIL_WATER_SEPARATOR.title}
            </h1>
            <p className="mt-6 text-sm leading-relaxed text-gray-600 sm:text-base">{OIL_WATER_SEPARATOR.intro}</p>
      ...
      <section className="bg-[#1a5276] py-12 text-white lg:py-16">
        <div className="mx-auto max-w-[1400px] px-4 lg:px-6">
          <motion.p
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            className="mb-10 text-center text-sm leading-relaxed text-white/90 sm:text-base lg:text-left"
          >
            {OIL_WATER_SEPARATOR.valueProposition}
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            {valueImages.map((src, i) => (
              <div key={src} className="product-img-pair-cell shadow-lg">
                <img
                  src={src}
                  alt={i === 0 ? "Horizontal oil water separator unit" : "Vertical OWS equipment"}
      ...
      <section className="bg-[#1a5276] py-12 text-center text-white lg:py-14">
        <div className="mx-auto max-w-[900px] px-4 lg:px-6">
          ...
          <p className="mt-4 text-sm leading-relaxed text-white/90 sm:text-base">
            Stop worrying about your wastewater problems &amp; focus on your business growth.
          </p>
          <Link
            to="/contact"
