...
import { PRESSURE_VESSELS } from "@/lib/products/pressure-vessels";

export default function PressureVesselsPage() {
  const { heroImages, valueImages, detailBlocks, vesselsOffered, faqs } = PRESSURE_VESSELS;
    ...
    <main className="bg-white text-gray-800">
      ...
      <div className="border-b border-gray-200 bg-[#f4f6f8]">
        <div className="mx-auto flex max-w-[1400px] items-center gap-2 px-4 py-3 text-xs font-medium uppercase tracking-wide text-gray-600 lg:px-6">
          ...
          <ChevronRight className="h-3.5 w-3.5 opacity-50" aria-hidden />
          <span className="text-[#1a5276]">Pressure Vessels</span>
        </div>
      </div>
      ...
      <section className="py-12 lg:py-16">
        <div className="mx-auto max-w-[1400px] px-4 lg:px-6">
          ...
          >
            <img
              src={heroImages.main}
              alt="Pressure vessel — cylindrical tank on supports"
            ...
            <div className="flex flex-col gap-3">
              {heroImages.secondary.map((src, i) => (
                <img
                  key={src}
                  src={src}
                  alt={i === 0 ? "Pressure vessel in workshop" : "Vertical pressure vessel"}
          ...
          >
            <h1 className="font-display text-2xl font-bold uppercase tracking-wide text-[#1a5276] sm:text-3xl lg:text-4xl">
              {PRESSURE_VESSELS.title}
            </h1>
            <p className="mt-6 text-sm leading-relaxed text-gray-600 sm:text-base">{PRESSURE_VESSELS.intro}</p>
      ...
      <section className="bg-[#1a5276] py-12 text-white lg:py-16">
        <div className="mx-auto grid max-w-[1400px] items-center gap-10 px-4 lg:grid-cols-2 lg:gap-14 lg:px-6">
          ...
          >
            {PRESSURE_VESSELS.valueProposition}
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            {valueImages.map((src, i) => (
              <div key={src} className="product-img-pair-cell shadow-lg">
                <img
                  src={src}
                  alt={i === 0 ? "Vertical storage tanks" : "Industrial treatment facility"}
      ...
      {/* Vessels we offer — single section only */}
      ...
      <section className="bg-[#1a5276] py-12 text-center text-white lg:py-14">
        <div className="mx-auto max-w-[900px] px-4 lg:px-6">
          ...
          <p className="mt-4 text-sm leading-relaxed text-white/90 sm:text-base">
            If you have any questions about our products or services, please feel free to contact us. We are always
            happy to help.
          </p>
          <Link
            to="/contact"
