...
import { REVERSE_OSMOSIS_PLANT } from "@/lib/products/reverse-osmosis-plant";

export default function ReverseOsmosisPlantPage() {
  const { heroImages, valueImages, detailBlocks, plantsOffered, faqs } = REVERSE_OSMOSIS_PLANT;
    ...
    <main className="bg-white text-gray-800">
      ...
      <div className="border-b border-gray-200 bg-[#f4f6f8]">
        <div className="mx-auto flex max-w-[1400px] items-center gap-2 px-4 py-3 text-xs font-medium uppercase tracking-wide text-gray-600 lg:px-6">
          ...
          <ChevronRight className="h-3.5 w-3.5 opacity-50" aria-hidden />
          <span className="text-[#1a5276]">Reverse Osmosis Plant</span>
        </div>
      </div>
      ...
      <section className="py-12 lg:py-16">
        <div className="mx-auto max-w-[1400px] px-4 lg:px-6">
          ...
          >
            <img
              src={heroImages.main}
              alt="Reverse osmosis plant — membrane housings and pumps"
            ...
            <div className="flex flex-col gap-3">
              {heroImages.secondary.map((src, i) => (
                <img
                  key={src}
                  src={src}
                  alt={i === 0 ? "RO filtration system" : "Industrial reverse osmosis equipment"}
          ...
          >
            ...
            <p className="mt-6 text-sm leading-relaxed text-gray-600 sm:text-base">{REVERSE_OSMOSIS_PLANT.intro}</p>
      ...
      <section className="bg-[#1a5276] py-12 text-white lg:py-16">
        <div className="mx-auto grid max-w-[1400px] items-center gap-10 px-4 lg:grid-cols-2 lg:gap-14 lg:px-6">
          ...
          >
            {REVERSE_OSMOSIS_PLANT.valueProposition}
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 16 }}
            whileInView={{ opacity: 1, y: 0 }}
          ...
          >
            {valueImages.map((src, i) => (
              <div key={src} className="product-img-pair-cell shadow-lg">
                <img
                  src={src}
                  alt={i === 0 ? "RO plant installation" : "Industrial water treatment facility"}
      ...
      <section className="border-t border-gray-100 bg-[#f4f6f8] py-12 lg:py-16">
        <div className="mx-auto max-w-[1400px] px-4 lg:px-6">
          <h2 className="text-center font-display text-xl font-bold uppercase text-[#1a5276] sm:text-2xl">
            Reverse Osmosis Plants We Offer
          </h2>
      ...
      <section className="bg-[#1a5276] py-12 text-center text-white lg:py-14">
        <div className="mx-auto max-w-[900px] px-4 lg:px-6">
          <h2 className="font-display text-xl font-bold uppercase leading-snug sm:text-2xl">
            Are you looking for a better water and effluent services?
          </h2>
          <Link
            to="/contact"
