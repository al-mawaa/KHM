...
import { SEAWATER_DESALINATION_PLANT } from "@/lib/products/seawater-desalination-plant";

export default function SeawaterDesalinationPlantPage() {
  const { heroImage, valueImages, detailBlocks, faqs } = SEAWATER_DESALINATION_PLANT;
    ...
    <main className="bg-white text-gray-800">
      ...
      <div className="border-b border-gray-200 bg-[#f4f6f8]">
        <div className="mx-auto flex max-w-[1400px] items-center gap-2 px-4 py-3 text-xs font-medium uppercase tracking-wide text-gray-600 lg:px-6">
          ...
          <ChevronRight className="h-3.5 w-3.5 opacity-50" aria-hidden />
          <span className="text-[#1a5276]">Seawater Desalination Plant</span>
        </div>
      </div>
      ...
      <section className="py-12 lg:py-16">
        <div className="mx-auto max-w-[1400px] px-4 lg:px-6">
          ...
          >
            <img
              src={heroImage}
              alt="Sea water RO desalination treatment plant — coastal facility"
          ...
          >
            <h1 className="font-display text-2xl font-bold uppercase tracking-wide text-[#1a5276] sm:text-3xl lg:text-4xl">
              {SEAWATER_DESALINATION_PLANT.title}
            </h1>
            <p className="mt-6 text-sm leading-relaxed text-gray-600 sm:text-base">{SEAWATER_DESALINATION_PLANT.intro}</p>
          ...
          >
            {valueImages.map((src, i) => (
              <div key={src} className="product-img-pair-cell shadow-lg">
                <img
                  src={src}
                  alt={i === 0 ? "Desalination plant piping and valves" : "SWRO filtration units"}
